...
const commandUrl = new URL('../src/owned-command.js', import.meta.url)
const preloadUrl = new URL('../build/preload/plugin-preload.cjs', import.meta.url)
...
test('keeps a fresh profile free of optional default plugins', async (t) => {
  const dshHome = temporaryDirectory(t)
  const result = await ensureDefaultPlugins({
    dshHome,
    pnpmEntry: '/pnpm.mjs',
    defaults: ['github:example/should-not-be-installed#v0.2.11'],
    runPnpmImpl: async () => assert.fail('pnpm should not run for optional defaults'),
  })
...
test('does not resurrect a previously removed optional plugin', async (t) => {
  const dshHome = temporaryDirectory(t)
  const profileDir = join(dshHome, 'profiles', 'web')
  const profileManifest = join(profileDir, 'package.json')
  writeJson(profileManifest, {
