...
test('Windows directory picker worker runs the packaged Electron executable as Node', () => {
  const hostPath = require.resolve('@deepseek-ai/dsh-host-directory-picker-native')
  const hostSource = readFileSync(hostPath, 'utf8')

  assert.match(hostSource, /ELECTRON_RUN_AS_NODE:\s*["']1["']/)
})
