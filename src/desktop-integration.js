...
export function resolveDesktopColorScheme({ preference, resolved }, shouldUseDarkColors) {
  if (preference === 'system') return shouldUseDarkColors ? 'dark' : 'light'
...
export function prepareHarnessToolchain({ directory, execPath, pnpmEntry, env = process.env, platform = process.platform }) {
  ...
  // Windows environment-variable names are case-insensitive, but Node may
  // expose the inherited key as `Path`. Returning both `Path` and `PATH` makes
  // the child-process winner dependent on key ordering and can silently put a
  // user/Corepack pnpm ahead of the desktop-owned launcher.
  const pathKey = Object.keys(env).find(key => key.toLowerCase() === 'path') ?? 'PATH'
  const inheritedPath = env[pathKey] ?? ''
  return { ...env, [pathKey]: `${directory}${delimiter}${inheritedPath}` }
}
...
/**
 * Ensure the app-owned integration exists for the web profile. User plugins
 * remain untouched; this only repairs the desktop bridge after a clean reset
 * or a release snapshot restore omitted the derived node_modules tree.
 */
export function installDesktopPluginForNewWebProfile({ sourceDir, dshHome, exists = existsSync }) {
  const webProfileDir = join(dshHome, 'profiles', 'web')
  const targetDir = join(dshHome, 'profiles', 'node_modules', '@dsh-desktop', 'integration')
  const complete = exists(webProfileDir) && DESKTOP_PLUGIN_FILES.every(file => {
    const source = join(sourceDir, file)
    const target = join(targetDir, file)
  ...
  if (complete) return { installed: false, profileDir: webProfileDir, targetDir }
  return {
    installed: true,
    repaired: exists(webProfileDir),
    profileDir: webProfileDir,
    targetDir: installDesktopPlugin({ sourceDir, dshHome }),
  }
}
...
export function readDesktopSettings(path) {
  try {
    ...
    const resolvedTheme = ['light', 'dark'].includes(parsed.resolvedTheme)
    ...
    return { editor: 'auto', theme: 'system' }
  }
}
