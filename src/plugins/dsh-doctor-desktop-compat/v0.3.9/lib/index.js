...
function resolveDshHome(env = process.env, home = homedir(), cwd = process.cwd()) {
	...
	const isPosix = home.startsWith("/") || raw !== void 0 && raw.startsWith("/");
	if (!raw) return isPosix ? join$1(home, ".dsh") : join(home, ".dsh");
	const expanded = raw === "~" ? home : raw.startsWith("~/") || raw.startsWith("~\\") ? isPosix ? join$1(home, raw.slice(2)) : join(home, raw.slice(2)) : raw;
	if (isPosix || expanded.startsWith("/")) return resolve$1(expanded.replaceAll("\\", "/"));
	return isAbsolute(expanded) ? resolve(expanded) : resolve(cwd, expanded);
}
function profileIdentity(dshHome, name, dshExecutable, role = "protected") {
	assertSafeProfileName(name);
	const canonicalHome = dshHome.startsWith("/") ? resolve$1(dshHome) : resolve(dshHome);
	const canonicalDsh = dshExecutable.startsWith("/") ? resolve$1(dshExecutable) : resolve(dshExecutable);
	return {
...
async function ensureDoctor(deps) {
	...
	const lifecycleEnv = {
		DSH_DOCTOR_PACKAGE_DIR: dirname(dirname(deps.cliPath)),
		...deps.dshScript ? { DSH_DOCTOR_DSH_SCRIPT: deps.dshScript } : {}
	};
	const first = await spawnImpl(process.execPath, [deps.cliPath, "service-install"], {
	...
	if (await (deps.capsuleStale ?? defaultCapsuleStale.bind(void 0, deps.paths))(deps.version, deps.source)) {
		const second = await spawnImpl(process.execPath, [deps.cliPath, "provision"], {
...
const RETRY_CODES = /* @__PURE__ */ new Set(["EPERM", "EACCES"]);
...
async function renameWithRetry(from, to) {
	for (let attempt = 0; attempt < RENAME_RETRIES; attempt++) try {
		...
		if (attempt < RENAME_RETRIES - 1) await setTimeout$1(RENAME_RETRY_DELAY_MS);
		else throw error;
	}
}
async function writeJsonAtomic(path, value, mode = 384) {
	...
	try {
		...
		if (RETRY_CODES.has(error.code ?? "")) try {
...
const DOCTOR_SETTINGS_NAMESPACE = "doctor";
function effectiveConfig(config) {
	return {
		...
		autoMigrate: config?.autoMigrate ?? DEFAULT_DOCTOR_POLICY.autoMigrate,
...
const apply = mountOnce("@linxin666/dsh-doctor", (ctx, config) => {
	...
	const baseLifecycle = serializeDoctorLifecycle(createDoctorLifecycle({
		paths,
		cliPath,
		dshScript: profile.dshExecutable,
		version: hostVersion,
	...
	ctx.inject(["settings"], (settingsCtx) => {
		settingsCtx.settings.installSection(ctx, DOCTOR_SETTINGS_NAMESPACE, Config, config ?? {}, {
			setSource: (source) => {
				current = source;
				sync();
			},
