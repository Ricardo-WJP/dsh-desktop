...
> DeepSeek Harness 目前仍处于开发者预览阶段，后续可能出现破坏兼容性的变更。DeepSeek Harness Desktop 会内置经过验证的固定 Harness 版本，既作为可复现的默认运行时，也作为更新失败时的恢复版本。

## 特性

- Release 安装包开箱即用，不要求用户另外安装 Node.js、npm 或执行 `npx`。
- 固定使用易于识别的本地地址 `http://127.0.0.1:3080`；若端口被其他程序占用，启动时会明确提示冲突，不会静默换端口。
...
如果 macOS 仍提示应用已损坏或无法验证开发者，请先将应用拖到 `/Applications`，确认安装包来自本仓库的 GitHub Release，然后在“终端”中仅为 DeepSeek Harness Desktop 清除隔离属性：
...
1. 启动 DeepSeek Harness Desktop，等待本地 Harness 服务就绪。
...
新安装默认只保留 Harness 必需的系统 Bundle，不预装任何可选插件，也不会在启动时自动恢复插件。用户可以在插件管理页自行搜索、安装、启用、更新和删除插件；桌面工作区的编辑器/文件夹操作属于应用自身适配层，不占用用户插件列表。
...
DSH 本身使用独立的更新通道。应用会比较当前运行的 `@deepseek-ai/dsh` 与 npm `latest` 版本，也可以通过**帮助 → 检查 DSH 更新**手动触发。确认更新后，新版本会安装到应用用户数据下的 `dsh-runtime` 目录，完整校验后才会激活，并自动重启本地 Harness；pnpm 会在安装时校验 npm 包的完整性数据。如果新版运行时无法启动，DeepSeek Harness Desktop 会自动停用它并退回内置版本；也可以通过**帮助 → 恢复内置 DSH**手动回退。
...
└─ 原生 DSH 工作区
   └─ 可选插件由原生 DSH / dsh-market 安装
```

打包 Profile 是纯白板：只包含 DSH 核心包，不预装任何可选插件。插件发现与安装留在原生 DSH 内，由用户自行安装和更新 `dsh-market` 及其他插件。桌面能力由本仓库内独立的双端适配层 `@dsh-desktop/integration` 提供；启动时应用只把这个内部适配层复制到 `$DSH_HOME/profiles/node_modules` 的上游扩展解析目录，再用一次性 `--patch` 加载。它不是用户可选插件，也不会修改内置的 `@deepseek-ai/dsh` CLI、外部 DSH 安装或用户的 `cordis.patch.yml`。可选的 DSH 在线更新只保存在应用自己的用户数据运行时中。
...
界面文件位置、哪些修改支持热更新以及打包前检查，见[界面修改指南](docs/ui-development.zh-CN.md)。
...
DeepSeek Harness Desktop 使用 [MIT License](LICENSE)。DeepSeek Harness 同样采用 MIT License，详见 [NOTICE.md](NOTICE.md)。
